...
struct ENGINE_API FVector
{
  public:
    ...
    //======================================//
  public:
    constexpr FVector() noexcept : X(0.f), Y(0.f), Z(0.f) {}
    ...
    explicit FVector(DirectX::FXMVECTOR InVector) noexcept : X(0.f), Y(0.f), Z(0.f)
    {
  ...
  public:
    ...
    constexpr bool operator!=(const FVector& Other) const noexcept { return !(*this == Other); }

    constexpr FVector operator-() const noexcept { return {-X, -Y, -Z}; }
  ...
  public:
    ...
    Float3 ToXMFLOAT3() const noexcept { return DirectX::XMFLOAT3(X, Y, Z); }

    // 현재 벡터를 DirectX::XMVECTOR 형식으로 변환함
    // W 성분은 기본값 0.f를 사용하며 필요 시 지정할 수 있음
    XMVector ToXMVector(float W = 0.f) const noexcept { return DirectX::XMVectorSet(X, Y, Z, W); }
    ...
    FVector GetSafeNormal2D(float Tolerance = 1.e-8f) const noexcept
    {
        ...
        if (SquareSum > Tolerance)
        {
            ...
            return FVector(DirectX::XMVectorGetX(Normalized), DirectX::XMVectorGetY(Normalized),
                           0.0f);
        }
