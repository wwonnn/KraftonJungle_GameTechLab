        ...
        if (ImGui::Button("Copy"))
        {
            ...
            auto FormatTable = [&](const char *Title, const TArray<FStatEntry> &Entries) {
                ...
                for (const FStatEntry &E : Sorted)
                {
                    if (E.CallCount == 0)
                        continue;
                    oss << E.Category << "\t" << E.Name << "\t" << E.CallCount << "\t" << E.TotalTime * 1000.0 << "\t"
                        << E.AvgTime * 1000.0 << "\t" << E.MaxTime * 1000.0 << "\t" << E.MinTime * 1000.0 << "\t"
                        << E.LastTime * 1000.0 << "\n";
                }
                oss << "\n";
            };
                                        ...
                                        bool &OutSortDescending, float TableHeight)
{
    ...
    auto SortPredicate = [&](const FStatEntry &A, const FStatEntry &B) -> bool {
        ...
        {
        case 0:
            return OutSortDescending ? (strcmp(A.Category, B.Category) > 0) : (strcmp(A.Category, B.Category) < 0);
        case 1:
            return OutSortDescending ? (strcmp(A.Name, B.Name) > 0) : (strcmp(A.Name, B.Name) < 0);
        case 2:
            ValA = (double)A.CallCount;
            ValB = (double)B.CallCount;
            break;
        case 3:
            ValA = A.TotalTime;
            ValB = B.TotalTime;
                          ...
                          ImVec2(0.0f, TableHeight)))
    {
        ...
        for (const FStatEntry &E : Entries)
        {
            ...
            ImGui::TableNextRow();
            ImGui::TableSetColumnIndex(0);
            ImGui::Text("%s", E.Category);
            ImGui::TableSetColumnIndex(1);
            ImGui::Text("%s", E.Name);
            ImGui::TableSetColumnIndex(2);
            ImGui::Text("%u", E.CallCount);
            ImGui::TableSetColumnIndex(3);
            ImGui::Text("%.3f", E.TotalTime * 1000.0);
            ImGui::TableSetColumnIndex(4);
            ImGui::Text("%.3f", E.AvgTime * 1000.0);
            ImGui::TableSetColumnIndex(5);
            ImGui::Text("%.3f", E.MaxTime * 1000.0);
            ImGui::TableSetColumnIndex(6);
            ImGui::Text("%.3f", E.MinTime * 1000.0);
            ImGui::TableSetColumnIndex(7);
            ImGui::Text("%.3f", E.LastTime * 1000.0);
        }
