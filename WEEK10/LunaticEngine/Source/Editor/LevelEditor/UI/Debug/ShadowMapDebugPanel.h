...
class FShadowMapDebugPanel : public FUIElement
{
  ...
  private:
    ...
    void RenderVizPass(ID3D11DeviceContext *DC, ID3D11ShaderResourceView *SrcSRV, bool bIsArray, uint32 SliceIndex,
                       float UVMinX, float UVMinY, float UVMaxX, float UVMaxY, float Brightness, uint32 Mode = 0,
                       float Exponent = 1.0f);
};
