...
static const ImU32 RegionColors[] = {
    ...
    IM_COL32(255, 130, 30, 220), IM_COL32(200, 80, 255, 220), IM_COL32(50, 230, 230, 220), IM_COL32(255, 80, 180, 220),
};
constexpr int32 NumRegionColors = (int32)(sizeof(RegionColors) / sizeof(RegionColors[0]));

static void DrawRegionOverlay(const TArray<FAtlasRegion> &Regions, float PreviewSize, float AtlasResolution,
                              uint32 FilterPageIdx, int32 HighlightLightIdx = -1)
{
...
void FShadowMapDebugPanel::EnsureVizRT(ID3D11Device *Dev, uint32 Size)
{
    ...
    if (FAILED(Dev->CreateTexture2D(&Desc, nullptr, &VizTexture)))
        return;
...
void FShadowMapDebugPanel::Render(float DeltaTime)
{
    ...
    if (SelectedTab == 0)
    {
        ...
        if (CSMCascadeIndex >= 0 && CSMCascadeIndex < (int32)MAX_SHADOW_CASCADES && SR.CSM.SRV)
        {
            ...
            RenderVizPass(DC, SR.CSM.SRV, true, CSMCascadeIndex, 0, 0, 1, 1, CSMDepthBrightness, (uint32)VizMode,
                          VizExponent);

            ImGui::Image((ImTextureID)VizSRV, ImVec2(PreviewSize, PreviewSize), ImVec2(0, 0), ImVec2(1, 1),
    ...
    else if (SelectedTab == 1)
    {
        ...
        if (SelLight.Type == ESelectedLightType::Spot && SelLight.EnvIndex >= 0 && pRegions && SR.Spot.SRV)
        {
            for (const FAtlasRegion &R : *pRegions)
            {
                ...
                RenderVizPass(DC, SR.Spot.SRV, true, R.PageIdx, uvMinX, uvMinY, uvMaxX, uvMaxY, SpotDepthBrightness,
                              (uint32)VizMode, VizExponent);
        ...
        if (!bShowCropped && SR.Spot.SRV)
        {
            ...
            {
                RenderVizPass(DC, SR.Spot.SRV, true, SpotPageIndex, 0, 0, 1, 1, SpotDepthBrightness, (uint32)VizMode,
                              VizExponent);
                ...
                if (bShowSpotRegions && pRegions)
                    DrawRegionOverlay(*pRegions, PreviewSize, static_cast<float>(SR.Spot.Resolution),
                                      static_cast<uint32>(SpotPageIndex));
            }
        }
    }
    // ============================================================
        ...
        if (SelLight.Type == ESelectedLightType::Point && SelLight.EnvIndex >= 0 && pRegions && SR.Point.SRV)
        {
            ...
            if (!FaceRegions.empty())
            {
                ...
                for (int32 f = 0; f < (int32)FaceRegions.size(); ++f)
                {
                    ...
                    ImGui::Image((ImTextureID)VizSRV, ImVec2(FacePreview, FacePreview), uv0, uv1, ImVec4(1, 1, 1, 1),
        ...
        if (!bShowCropped && SR.Point.SRV)
        {
            ...
            if (bShowPointRegions && pRegions)
                DrawRegionOverlay(*pRegions, PreviewSize, static_cast<float>(SR.Point.Resolution),
                                  static_cast<uint32>(PointPageIndex));
        }
    }
