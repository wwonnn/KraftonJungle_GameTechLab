...
uint64 FEditorViewerWindowWidget::ComputeEditableMeshSignature(const FSkeletalMesh* MeshData) const
{
	...
	for (const FBoneInfo& Bone : Bones)
	{
		...
		Hash = HashString(Hash, Bone.ExportName);
...
void FEditorViewerWindowWidget::RenderBoneDetails(USkeletalMeshComponent* SkelComp)
{
    ...
    const TArray<FBoneInfo>& Bones = SkelComp->GetSkeletalMesh()->GetMeshData()->GetBones();
    if (SelectedBoneIndex < 0 || SelectedBoneIndex >= static_cast<int32>(Bones.size())) return;

    const FBoneInfo& Bone = Bones[SelectedBoneIndex];
    const FString BoneName = Bone.Name.ToString();
    ImGui::Text("Bone: %s (Index: %d)", BoneName.c_str(), SelectedBoneIndex);
    ImGui::Spacing();
...
void FEditorViewerWindowWidget::RebuildBoneToSocketIndices(const FSkeletalMesh* MeshData)
{
    ...
    const int32 BoneCount = static_cast<int32>(MeshData->GetBones().size());
...
{
    if (!CachedMesh) return;
    if (BoneIdx < 0 || BoneIdx >= static_cast<int32>(CachedMesh->GetBones().size())) return;
...
void FEditorViewerWindowWidget::DrawSocketInspector()
{
    ...
    // Bone 콤보
    const TArray<FBoneInfo>& Bones = CachedMesh->GetBones();
