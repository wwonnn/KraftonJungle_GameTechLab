#include "InlineFloatCurveEditor.h"
...
namespace
{
        ...
        const ImVec2& Max)
    {
        ...
        const ImVec2 KeyPos = CurveToScreen(Key.Time, Key.Value, ViewMinTime, ViewMaxTime, ViewMinValue, ViewMaxValue, Min, Max);
        return ImVec2(KeyPos.x + DirectionVector.x * TangentHandleLength, KeyPos.y + DirectionVector.y * TangentHandleLength);
    }

    static void ExpandRange(float& MinValue, float& MaxValue)
    {
        if (MaxValue <= MinValue + TimeEpsilon)
        {
            MinValue -= 0.5f;
            MaxValue += 0.5f;
        }
...
void FInlineFloatCurveEditor::ResetSelection()
{
    SelectedKeyIndex = -1;
    bDraggingSelectedKey = false;
    DraggingTangentHandle = ETangentHandle::None;
    bPanningView = false;
    bZoomSelecting = false;
    bSuppressNextCanvasContextMenu = false;
    bWasEditingLastFrame = false;
}

void FInlineFloatCurveEditor::FitHorizontalToCurve(const FFloatCurve& Curve)
{
    if (Curve.Keys.empty())
    {
        ViewMinTime = 0.0f;
    ...
    ExpandRange(ViewMinTime, ViewMaxTime);
}

void FInlineFloatCurveEditor::FitVerticalToCurve(const FFloatCurve& Curve)
{
    if (Curve.Keys.empty())
    {
        ViewMinValue = -1.0f;
    ...
    ExpandRange(ViewMinValue, ViewMaxValue);
}

void FInlineFloatCurveEditor::FitViewToCurve(const FFloatCurve& Curve)
{
    FitHorizontalToCurve(Curve);
    FitVerticalToCurve(Curve);
}

bool FInlineFloatCurveEditor::HasSelectedKey(const FFloatCurve& Curve) const
{
    return SelectedKeyIndex >= 0 && SelectedKeyIndex < static_cast<int32>(Curve.Keys.size());
}

void FInlineFloatCurveEditor::SetSelectedKeyInterpMode(FFloatCurve& Curve, ECurveInterpMode Mode)
{
    if (!HasSelectedKey(Curve)) return;
    Curve.Keys[SelectedKeyIndex].InterpMode = Mode;
    if (Mode == ECurveInterpMode::Cubic)
    {
...
void FInlineFloatCurveEditor::SetSelectedKeyTangentMode(FFloatCurve& Curve, ECurveTangentMode Mode)
{
    if (!HasSelectedKey(Curve)) return;
    Curve.Keys[SelectedKeyIndex].TangentMode = Mode;
    if (Mode == ECurveTangentMode::Auto)
    {
        Curve.AutoSetTangents();
    }
}

void FInlineFloatCurveEditor::FlattenSelectedKeyTangents(FFloatCurve& Curve)
{
    if (!HasSelectedKey(Curve)) return;
...
void FInlineFloatCurveEditor::StraightenSelectedKeyTangents(FFloatCurve& Curve)
{
    if (!HasSelectedKey(Curve)) return;

    FCurveKey& Key = Curve.Keys[SelectedKeyIndex];
    float Slope = 0.0f;
    if (SelectedKeyIndex > 0 && SelectedKeyIndex + 1 < static_cast<int32>(Curve.Keys.size()))
    {
        const FCurveKey& Prev = Curve.Keys[SelectedKeyIndex - 1];
        const FCurveKey& Next = Curve.Keys[SelectedKeyIndex + 1];
        const float Dt = Next.Time - Prev.Time;
        Slope = std::fabs(Dt) < TimeEpsilon ? 0.0f : (Next.Value - Prev.Value) / Dt;
    }
    else if (SelectedKeyIndex > 0)
    {
        const FCurveKey& Prev = Curve.Keys[SelectedKeyIndex - 1];
        const float Dt = Key.Time - Prev.Time;
        Slope = std::fabs(Dt) < TimeEpsilon ? 0.0f : (Key.Value - Prev.Value) / Dt;
    }
    else if (SelectedKeyIndex + 1 < static_cast<int32>(Curve.Keys.size()))
    {
        const FCurveKey& Next = Curve.Keys[SelectedKeyIndex + 1];
        const float Dt = Next.Time - Key.Time;
        Slope = std::fabs(Dt) < TimeEpsilon ? 0.0f : (Next.Value - Key.Value) / Dt;
    }
...
FInlineCurveEditResult FInlineFloatCurveEditor::Render(
    const char* Id,
    FFloatCurve& Curve,
    const ImVec2& InCanvasSize,
    EInteractionMode Mode,
    bool bShowInspector)
{
    FInlineCurveEditResult Result;

    ImGui::PushID(Id);

    const float SafeWidth = (std::max)(InCanvasSize.x, 32.0f);
    const float SafeHeight = (std::max)(InCanvasSize.y, 32.0f);
    const ImVec2 CanvasSize(SafeWidth, SafeHeight);
    ...
    DrawList->AddRectFilled(CanvasMin, CanvasMax, IM_COL32(17, 18, 22, 255), 4.0f);
    DrawList->AddRect(CanvasMin, CanvasMax, IM_COL32(57, 60, 70, 255), 4.0f);

    for (int GridIndex = 1; GridIndex < 10; ++GridIndex)
    {
        const float T = static_cast<float>(GridIndex) / 10.0f;
        const ImU32 GridColor = (GridIndex == 5) ? IM_COL32(52, 55, 64, 255) : IM_COL32(35, 37, 44, 255);
    ...
    if (!Curve.IsEmpty())
    {
        ...
        ImVec2 PreviousPoint = CurveToScreen(ViewMinTime, Curve.Evaluate(ViewMinTime), ViewMinTime, ViewMaxTime, ViewMinValue, ViewMaxValue, CanvasMin, CanvasMax);
        for (int32 SampleIndex = 1; SampleIndex < SampleCount; ++SampleIndex)
        {
            ...
            const ImVec2 CurrentPoint = CurveToScreen(SampleTime, Curve.Evaluate(SampleTime), ViewMinTime, ViewMaxTime, ViewMinValue, ViewMaxValue, CanvasMin, CanvasMax);
    ...
    const bool bHasSelectedKey = HasSelectedKey(Curve);
    const bool bCanShowArriveHandle = bHasSelectedKey && SelectedKeyIndex > 0 && Curve.Keys[SelectedKeyIndex - 1].InterpMode == ECurveInterpMode::Cubic;
    const bool bCanShowLeaveHandle = bHasSelectedKey && SelectedKeyIndex + 1 < static_cast<int32>(Curve.Keys.size()) && Curve.Keys[SelectedKeyIndex].InterpMode == ECurveInterpMode::Cubic;

    if (bHasSelectedKey && (bCanShowArriveHandle || bCanShowLeaveHandle))
    {
        const FCurveKey& Key = Curve.Keys[SelectedKeyIndex];
        const ImVec2 KeyPos = CurveToScreen(Key.Time, Key.Value, ViewMinTime, ViewMaxTime, ViewMinValue, ViewMaxValue, CanvasMin, CanvasMax);

        if (bCanShowArriveHandle)
        {
            const ImVec2 HandlePos = GetTangentHandlePosition(Key, true, ViewMinTime, ViewMaxTime, ViewMinValue, ViewMaxValue, CanvasMin, CanvasMax);
            if (IsFinitePoint(HandlePos))
            {
                if (IsPointNear(IO.MousePos, HandlePos, TangentHandleHitRadius)) HoveredTangentHandle = ETangentHandle::Arrive;
        ...
        {
            const ImVec2 HandlePos = GetTangentHandlePosition(Key, false, ViewMinTime, ViewMaxTime, ViewMinValue, ViewMaxValue, CanvasMin, CanvasMax);
            if (IsFinitePoint(HandlePos))
            {
                if (IsPointNear(IO.MousePos, HandlePos, TangentHandleHitRadius)) HoveredTangentHandle = ETangentHandle::Leave;
                DrawList->AddLine(KeyPos, HandlePos, IM_COL32(95, 150, 255, 180), 1.5f);
                DrawList->AddCircleFilled(HandlePos, 4.5f, IM_COL32(95, 150, 255, 255));
                DrawList->AddCircle(HandlePos, 4.5f, IM_COL32(15, 20, 30, 220), 12, 1.0f);
            }
    ...
    for (int32 KeyIndex = 0; KeyIndex < static_cast<int32>(Curve.Keys.size()); ++KeyIndex)
    {
        ...
        if ((DX * DX + DY * DY) <= KeyHitRadius * KeyHitRadius) HoveredKeyIndex = KeyIndex;
    ...
    if (bCanvasHovered && IO.MouseWheel != 0.0f)
    {
        ...
        if (bZoomTime)
        {
            ...
            if (ViewMaxTime <= ViewMinTime + TimeEpsilon) ViewMaxTime = ViewMinTime + TimeEpsilon;
        }
        if (bZoomValue)
        {
            ViewMinValue = MouseValue + (ViewMinValue - MouseValue) * Zoom;
            ViewMaxValue = MouseValue + (ViewMaxValue - MouseValue) * Zoom;
            if (ViewMaxValue <= ViewMinValue + TimeEpsilon) ViewMaxValue = ViewMinValue + TimeEpsilon;
        }
    }
    ...
    {
        if (Mode == EInteractionMode::Zoom)
        {
            bZoomSelecting = true;
            ZoomStart = IO.MousePos;
            ZoomEnd = IO.MousePos;
        }
        else
        {
    ...
    if (DraggingTangentHandle != ETangentHandle::None && HasSelectedKey(Curve) && ImGui::IsMouseDragging(ImGuiMouseButton_Left))
    {
        FCurveKey& Key = Curve.Keys[SelectedKeyIndex];
        float MouseTime = 0.0f;
        float MouseValue = 0.0f;
        ...
        if (DraggingTangentHandle == ETangentHandle::Arrive)
        {
            ...
            NewTangent = std::fabs(DeltaTime) > TimeEpsilon ? (Key.Value - MouseValue) / DeltaTime : Key.ArriveTangent;
        }
        else
        {
            const float DeltaTime = MouseTime - Key.Time;
            NewTangent = std::fabs(DeltaTime) > TimeEpsilon ? (MouseValue - Key.Value) / DeltaTime : Key.LeaveTangent;
        }

        if (Key.TangentMode == ECurveTangentMode::Auto) Key.TangentMode = ECurveTangentMode::User;

        if (Key.TangentMode == ECurveTangentMode::Break)
        {
            if (DraggingTangentHandle == ETangentHandle::Arrive) Key.ArriveTangent = NewTangent;
    ...
    if (bDraggingSelectedKey && HasSelectedKey(Curve) && ImGui::IsMouseDragging(ImGuiMouseButton_Left))
    {
        FCurveKey& Key = Curve.Keys[SelectedKeyIndex];
        ...
        if (SelectedKeyIndex > 0) Key.Time = (std::max)(Key.Time, Curve.Keys[SelectedKeyIndex - 1].Time + TimeEpsilon);
        if (SelectedKeyIndex + 1 < static_cast<int32>(Curve.Keys.size())) Key.Time = (std::min)(Key.Time, Curve.Keys[SelectedKeyIndex + 1].Time - TimeEpsilon);
        Result.bChanged = true;
    }

    if (bZoomSelecting && ImGui::IsMouseDragging(ImGuiMouseButton_Left, 3.0f))
    {
        ZoomEnd = IO.MousePos;
        const ImVec2 Min((std::min)(ZoomStart.x, ZoomEnd.x), (std::min)(ZoomStart.y, ZoomEnd.y));
        const ImVec2 Max((std::max)(ZoomStart.x, ZoomEnd.x), (std::max)(ZoomStart.y, ZoomEnd.y));
        DrawList->AddRectFilled(Min, Max, IM_COL32(74, 144, 255, 35));
        DrawList->AddRect(Min, Max, IM_COL32(74, 144, 255, 200));
    }
    ...
    if (ImGui::IsMouseReleased(ImGuiMouseButton_Left))
    {
        ...
        {
            ZoomEnd = IO.MousePos;
            if (std::fabs(ZoomEnd.x - ZoomStart.x) > 8.0f && std::fabs(ZoomEnd.y - ZoomStart.y) > 8.0f)
            {
                float T0 = 0.0f, V0 = 0.0f, T1 = 0.0f, V1 = 0.0f;
                ScreenToCurve(ZoomStart, ViewMinTime, ViewMaxTime, ViewMinValue, ViewMaxValue, CanvasMin, CanvasMax, T0, V0);
                ScreenToCurve(ZoomEnd, ViewMinTime, ViewMaxTime, ViewMinValue, ViewMaxValue, CanvasMin, CanvasMax, T1, V1);
                ViewMinTime = (std::min)(T0, T1);
                ViewMaxTime = (std::max)(T0, T1);
                ViewMinValue = (std::min)(V0, V1);
                ViewMaxValue = (std::max)(V0, V1);
                ExpandRange(ViewMinTime, ViewMaxTime);
                ExpandRange(ViewMinValue, ViewMaxValue);
            }
        }
    ...
    if (ImGui::BeginPopup("CanvasContext"))
    {
        ...
        if (ImGui::MenuItem("Fit To Keys")) FitViewToCurve(Curve);
        ImGui::EndPopup();
    }

    if (HoveredTangentHandle != ETangentHandle::None && HasSelectedKey(Curve))
    {
        const FCurveKey& Key = Curve.Keys[SelectedKeyIndex];
    ...
    DrawList->AddText(ImVec2(CanvasMin.x + 4.0f, CanvasMax.y - 16.0f), IM_COL32(122, 128, 140, 255), "RMB: menu / pan, Wheel: zoom");

    const bool bEditingNow = bDraggingSelectedKey || DraggingTangentHandle != ETangentHandle::None || ImGui::IsItemActive();
    Result.bEditStarted = bEditingNow && !bWasEditingLastFrame;
    Result.bEditEnded = !bEditingNow && bWasEditingLastFrame;
    bWasEditingLastFrame = bEditingNow;
    ...
    if (bShowInspector)
    {
        ...
        {
            if (ImGui::DragFloat("Default Value", &Curve.DefaultValue, 0.01f)) Result.bChanged = true;

            ImGui::Separator();
            ImGui::TextUnformatted("View");
            if (ImGui::Button("Fit To Keys")) FitViewToCurve(Curve);
            if (ImGui::DragFloat("Min Time", &ViewMinTime, 0.01f)) ViewMaxTime = (std::max)(ViewMaxTime, ViewMinTime + TimeEpsilon);
            if (ImGui::DragFloat("Max Time", &ViewMaxTime, 0.01f)) ViewMaxTime = (std::max)(ViewMaxTime, ViewMinTime + TimeEpsilon);
            if (ImGui::DragFloat("Min Value", &ViewMinValue, 0.01f)) ViewMaxValue = (std::max)(ViewMaxValue, ViewMinValue + TimeEpsilon);
            if (ImGui::DragFloat("Max Value", &ViewMaxValue, 0.01f)) ViewMaxValue = (std::max)(ViewMaxValue, ViewMinValue + TimeEpsilon);
            ...
            for (int32 KeyIndex = 0; KeyIndex < static_cast<int32>(Curve.Keys.size()); ++KeyIndex)
            {
                ...
                if (ImGui::Selectable(Label, SelectedKeyIndex == KeyIndex)) SelectedKeyIndex = KeyIndex;
            }
            ...
            if (HasSelectedKey(Curve))
            {
                ...
                {
                    SetSelectedKeyInterpMode(Curve, static_cast<ECurveInterpMode>(CurrentInterpMode));
                    Result.bChanged = true;
                }
                ...
                {
                    SetSelectedKeyTangentMode(Curve, static_cast<ECurveTangentMode>(CurrentTangentMode));
                    Result.bChanged = true;
                }

                if (Key.TangentMode != ECurveTangentMode::Auto)
                {
                    if (ImGui::DragFloat("Arrive Tangent", &Key.ArriveTangent, 0.01f)) Result.bChanged = true;
                    if (ImGui::DragFloat("Leave Tangent", &Key.LeaveTangent, 0.01f)) Result.bChanged = true;
                }
                ...
                if (ImGui::DragFloat("Value", &Key.Value, 0.01f)) Result.bChanged = true;
            }
            else
            {
